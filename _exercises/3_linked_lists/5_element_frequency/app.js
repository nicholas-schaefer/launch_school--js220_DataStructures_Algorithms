...
function traverseList(list, reducer, initialValue){
  let head = list;
...
function createFrequencyList(list){
  const reducer = ((acc, curr) => {
    acc.has(curr) || acc.set(curr, 0);
    acc.set(curr, acc.get(curr) + 1);
    return acc;
  })
  const initialValue = new Map();
  const frequencyMap = traverseList(list, reducer, initialValue);

  return createLinkedList(Array.from(frequencyMap.values()))
}
